      ...
      <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content p-3" style="border-radius: 20px;">
          ...
          <div class="modal-header d-flex justify-content-center align-items-center align-content-center btn-primary" style="border-bottom: none; border-top-left-radius: none; border-top-right-radius: none ">
            <h5 class="modal-title text-center ps-4 text-white" id="exampleModalLabel" style="font-weight: 700; ">Add Booth</h5>
          ...
          <div class="modal-body">
            <div class="card" style="border:none; box-shadow:none !important;">
              <div class="col-xl-12 mx-auto">
                <form method="post" action="<?php echo e(url('booth')); ?>" class="form">
                  <?php echo csrf_field(); ?>
                  <div class="listings-container  px-3 my-2">
                    ...
                    <div class="desgin-form d-flex setting flex-row modal-text my-4">
                      ...
                      <select name="panchayat_name" id="panchayat_container" class="form-control panchayat_container ls_name w-75" data-type="mahanagar-panchayat" required>
                        <option value="selected disabled">Choose Mahanagar/Panchayat</option>
                      </select>
                    </div>
                    <div class="desgin-form d-flex setting flex-row modal-text my-4">
                      <h5 class="form_heading modal-text w-25">Ward/Village Choose</h5>
                      <select name="ward_village" id="ward_village" class="form-control ward_village ls_name w-75" data-type="village" required>
                        <option value="selected disabled">Choose Ward/Village </option>
                      </select>
                    </div>
                    <div class="desgin-form d-flex setting flex-row modal-text my-4">
                      <h5 class="form_heading modal-text w-25">Ward/Village Name</h5>
                      <select name="village_name" id="village_container" class="form-control village_container w-75" required>
                        <option value="selected disabled">Choose village</option>
                      </select>
                    </div>
                    <div class="desgin-form d-flex setting flex-row modal-text my-4">
                      <h5 class="form_heading modal-text w-25">Booth Name</h5>
                      <input type="text" name="booth_name" id="booth_name" class="form-control custom-input form_input modal-text w-75" required>
                    </div>
  ...
  <div class="col-lg-12 px-lg-0">
    <div class="card mb-4">
      <div class="card-header pb-0">
        <h6 style="font-size: 1.5rem;font-weight:bold;">Booth Table</h6>
      </div>
          ...
          <table class="table align-items-center justify-content-center mb-0">
            <thead style="background: #7D6F6C;">
              <tr>
                ...
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 w-25 text-center text-white">Village/Ward </th>
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 w-25 text-center text-white">Village name</th>
                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 w-25 text-center text-white">Booth name</th>
            ...
            <tbody>
              <?php $__currentLoopData = $booth; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=> $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <tr class="table-form">
                <form method="post" action="<?php echo e(url('edit-booth').'/'.$item->id); ?>" class="form-id">
                  <?php echo csrf_field(); ?>
                  <td class="text-center">
                    <div class="d-flex px-2">
                      <div class="my-auto">
                        <a href="<?php echo e(url('lok-vidhan').'/'.$item->loksabha_id); ?>" target="blank"><label class="pointer"><?php echo e($item->get_loksabha->ls_name ?? ''); ?></label></a>

                      </div>
                    </div>
                  </td>
                  <td class="text-center">
                    <a href="<?php echo e(url('vidhan-block').'/'.$item->vidhansabha_id); ?>" target="blank"><label class="pointer"><?php echo e($item->get_vidhansabha->vs_name ?? ''); ?></label></a>

                  </td>
                  <td class="text-center">
                    <a href="<?php echo e(url('block-panchayat').'/'.$item->block_id); ?>" target="_blank"><label class="pointer"><?php echo e($item->get_block->block_name ?? ''); ?></label></a>

                  </td>
                  <td class="text-center">
                    <a href="<?php echo e(url('panchayat-village').'/'.$item->panchayat_id); ?>" target="blank"><label class="pointer"><?php echo e($item->get_panchayat->panchayat_name ?? ''); ?></label>
                  ...
                  <td class="text-center">
                    <a href="<?php echo e(url('village-booth').'/'.$item->village_id); ?>" target="blank"><label class="pointer"><?php echo e($item->get_village->village_name ?? ''); ?></label></a>
                  </td>
                  <td class="text-center">
                    <label class="input_label"><?php echo e($item->booth_name ?? ''); ?></label>
                    <input type="text" name="booth_name" class="form-control form_input d-none" value="<?php echo e($item->booth_name ?? ''); ?>">
                  </td>
                  <td class="actions text-center">
                    ...
                    <div class="table-action" style="display: block;">
                      ...
                      <a href="<?php echo e(url('delete-booth').'/'.$item->id); ?>" class="emp_delete" data-toggle="tooltip" data-placement="bottom" title="Delete"><img src="<?php echo e(asset('/assets/img/delete.png')); ?>" alt="" class="img-fluid mx-1"></a>
                    </div>
                </form>
                </td>
                <?php if($item->booth_status==2): ?>
                <td class="text-center">
                  <form method="GET" action="<?php echo e(url('booth-active').'/'.$item->id); ?>">
                    <input type="checkbox" data-toggle="toggle" data-on=" " data-off=" " data-style="ios" data-key="check<?php echo e($key); ?>" checked>
                  </form>
                </td>
                <?php else: ?>
                <td class="text-center">
                  <form method="GET" action="<?php echo e(url('booth-deactive').'/'.$item->id); ?>">
      ...
      <div class="d-flex flex-column justify-content-center align-items-center align-content-center mb-1 mt-1 mx-auto">
        <?php echo e($booth->links('pagination::bootstrap-4')); ?>

        <!-- <?php echo e($booth->links('pagination::simple-tailwind')); ?> -->
        <!-- <?php echo e($booth->links('pagination::semantic-ui')); ?>
...
<?php echo $__env->make('Admin.layouts.user_type.auth', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/chiraggupta/Sites/localhost/chirag/srchout/infospeed_admin/resources/views/Admin/booth/booth.blade.php ENDPATH**/ ?>