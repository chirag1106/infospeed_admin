          ...
          <div class="modal-body">
            <div class="card" style="border:none; box-shadow: none !important;">
              <div class="col-xl-12 mx-auto">
                <form method="post" action="{{ url('block') }}" class="form">
                  @csrf
        ...
        <div class="" style="overflow-x:auto !important;">
          <table class="table align-items-center justify-content-center mb-0">
            ...
            <tbody>
              ...
              <tr class="table-form">
                <form method="post" action="{{url('edit-block').'/'.$item->id}}" class="form-id">
                  @csrf
                  <td>
                    <div class=" px-2">
                      <div class="my-auto">
                        <a href="{{url('lok-vidhan').'/'.$item->loksabha_id}}" target="blank" class="input_label pointer"><label class=" input_label pointer">{{ $item->get_loksabha->ls_name ?? ''}}</label></a>
                      </div>
                    </div>
                  </td>
                  <td>
                  ...
                  <td>
                    ...
                    <input type="text" class=" form-control text-sm font-weight-bold mb-0 form_input text-capitalize  d-none @error('block_name') is-invalid @enderror" style="width:100%; text-transform: capatilize!important;" name="block_name" value="{{$item->block_name ?? ''}}" required>
                  </td>
                  <td class="actions">
                    ...
                    <div class="table-action text-center" style="display: block;">
                      ...
                      <a href="{{url('delete-block').'/'.$item->id}}" class="emp_delete mb-0" data-toggle="tooltip" data-placement="bottom" title="Delete"><img src="{{ asset('/assets/img/delete.png') }}" alt="" class="img-fluid mx-1"></a>
                    </div>
                </form>
                </td>
                @if($item->block_status==2)
                <td class="text-center">
                  <form method="GET" action="{{url('/')}}/block-active/{{$item->id ?? ''}}">
                    <input type="checkbox" data-toggle="toggle" data-on=" " data-off=" " data-style="ios" data-key="check{{ $key }}" checked>
                  </form>
                </td>
                @else($item->block_status==1)
                <td class="text-center">
                  <form method="GET" action="{{url('/')}}/block-deactive/{{$item->id ?? ''}}">
