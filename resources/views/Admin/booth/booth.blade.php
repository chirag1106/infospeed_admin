          ...
          <div class="modal-body">
            <div class="card" style="border:none; box-shadow:none !important;">
              <div class="col-xl-12 mx-auto">
                <form method="post" action="{{url('booth')}}" class="form">
                  @csrf
  ...
  <div class="col-lg-12 px-lg-0">
    <div class="card mb-4">
        ...
        <div class="" style="overflow-x:auto !important;">
          <table class="table align-items-center justify-content-center mb-0">
            ...
            <tbody>
              ...
              <tr class="table-form">
                <form method="post" action="{{url('edit-booth').'/'.$item->id}}" class="form-id">
                  @csrf
                  <td class="text-center">
                    <div class="d-flex px-2">
                      <div class="my-auto">
                        <a href="{{url('lok-vidhan').'/'.$item->loksabha_id}}" target="blank"><label class="pointer">{{ $item->get_loksabha->ls_name ?? ''}}</label></a>

                      </div>
                    </div>
                  </td>
                  <td class="text-center">
                    <a href="{{url('vidhan-block').'/'.$item->vidhansabha_id}}" target="blank"><label class="pointer">{{ $item->get_vidhansabha->vs_name ?? ''}}</label></a>

                  </td>
                  <td class="text-center">
                    <a href="{{url('block-panchayat').'/'.$item->block_id}}" target="_blank"><label class="pointer">{{ $item->get_block->block_name ?? ''}}</label></a>

                  </td>
                  <td class="text-center">
                    <a href="{{url('panchayat-village').'/'.$item->panchayat_id}}" target="blank"><label class="pointer">{{ $item->get_panchayat->panchayat_name ?? ''}}</label>
                  ...
                  <td class="text-center">
                    <a href="{{url('village-booth').'/'.$item->village_id}}" target="blank"><label class="pointer">{{ $item->get_village->village_name ?? ''}}</label></a>
                  </td>
                  <td class="text-center">
                    <label class="input_label">{{ $item->booth_name ?? ''}}</label>
                    <input type="text" name="booth_name" class="form-control form_input d-none" value="{{ $item->booth_name ?? ''}}">
                  </td>
                  <td class="actions text-center">
                    ...
                    <div class="table-action" style="display: block;">
                      ...
                      <a href="{{url('delete-booth').'/'.$item->id}}" class="emp_delete" data-toggle="tooltip" data-placement="bottom" title="Delete"><img src="{{ asset('/assets/img/delete.png') }}" alt="" class="img-fluid mx-1"></a>
                    </div>
                </form>
                </td>
                @if($item->booth_status==2)
                <td class="text-center">
                  <form method="GET" action="{{url('booth-active').'/'.$item->id}}">
                    <input type="checkbox" data-toggle="toggle" data-on=" " data-off=" " data-style="ios" data-key="check{{ $key }}" checked>
                  </form>
                </td>
                @else($item->booth_status==1)
                <td class="text-center">
                  <form method="GET" action="{{url('booth-deactive').'/'.$item->id}}">
      ...
      <div class="d-flex flex-column justify-content-center align-items-center align-content-center mb-1 mt-1 mx-auto">
        {{$booth->links('pagination::bootstrap-4')}}
        <!-- {{ $booth->links('pagination::simple-tailwind') }} -->
        <!-- {{ $booth->links('pagination::semantic-ui') }}
              {{ $booth->links('pagination::simple-tailwind') }} -->
      </div>
    </div>
  </div>
