      ...
      <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content p-3" style="border-radius: 20px;">
          ...
          <div class="modal-header d-flex justify-content-center align-items-center align-content-center btn-primary text-white" style="border-bottom: none; border-top-left-radius: none; border-top-right-radius: none ">
            <h5 class="modal-title text-center ps-4 text-white" style="text-transform:capitalize;" id="exampleModalLabel" style="font-weight: 700; ">{{$village->village_name ?? ''}}</h5>
          ...
          <div class="modal-body">
            <div class="card" style="border:none; box-shadow:none !important">
              <div class="col-xl-12 mx-auto">
                <form method="post" action="{{ url('village-booth') }}">
                  @csrf
                  <div class="listings-container  px-3 my-2">
                    <input type="hidden" name="loksabha_id" value="{{$village->loksabha_id ?? ''}}">
                    <input type="hidden" name="vidhansabha_id" value="{{$village->vidhansabha_id ?? ''}}">
                    <input type="hidden" name="block_id" value="{{$village->block_id ?? ''}}">
                    <input type="hidden" name="panchayat_id" value="{{$village->panchayat_id?? ''}}">
                    <input type="hidden" name="village_id" value="{{$village->id?? ''}}">
                    <input type="hidden" name="ward_village" value="{{$village->village_choosing?? ''}}">
  ...
  <div class="col-lg-12 p-0">
    <div class="card mb-4">
      <div class="card-header pb-0">
        <h6 style="font-size: 1.5rem; font-weight:bold; text-transform:capitilize"> {{$village->village_name ?? ''}}</h6>
      </div>
        ...
        <div class="table-responsive p-0">
          <table class="table table-responsive align-items-center justify-content-center mb-0">
            ...
            <tbody>
              ...
              <tr class="table-form">
                <form method="post" action="{{url('edit-booth').'/'.$item->id}}" id="form-id">
                  @csrf
                  {{-- <td>
                  ...
                  <td class="text-center">
                    <div class="d-flex align-items-center px-2">
                      <div class="my-auto">
                        ...
                        <input type="text" class="form-control form_input d-none text-capitalize" name="booth_name" style="width:100%" value="{{ $item->booth_name ?? ''}}" required>
                      </div>
                    </div>
                  </td>
                  <td class="actions text-center">
                    ...
                    <div class="table-action text-center" style="display: block;">
                      <a href="javascript:;" class="list_edit"><img src="{{ asset('/assets/img/edit.png') }}" alt="" class="img-fluid mx-1"></a>
                      <a href="{{url('delete-booth').'/'.$item->id}}" class="emp_delete"><img src="{{ asset('/assets/img/delete.png') }}" alt="" class="img-fluid mx-1"></a>
                    </div>
                  </td>
                </form>
                {{-- @if($item->status==2)
